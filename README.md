...
* Hyper: Now uses the right ⌃ (rctrl) and ⌘ (rcmd) keys, while using the left keys for Shift (lshift) and ⌥ (lalt).
* Meh: Also uses the right ⌃ key (rctrl), with Shift and ⌥ utilizing the left keys.

This means you can now have keybinds like `hyper + lcmd + rshift +ralt - t : ...` if you want more granularity.
